...
setwd('D:/work_data/raptor/unmarked_model_no_first_year_cov_1km/')
all_sp<-Sys.glob('*.RData')



func_get_coef<-function(sp) { 
  # sp=17

  target_file<-paste0('D:/work_data/raptor/unmarked_model_no_first_year_cov_1km_coefs_confs/',gsub('.RData','.csv',all_sp[sp]))
...
foreach(
  sp=1:length(all_sp),
  .errorhandling = 'stop',
  .packages = c('unmarked','dplyr')
) %dopar% func_get_coef(sp)
stopCluster(cl)
...
for (s in 1:length(all_sp)) {
  ...
  # s=17
  target_file<-paste0('D:/work_data/raptor/unmarked_model_sensitivity_no_first_year_cov_1km/',gsub('.RData','.csv',all_sp[s]))
  
  if(file.exists(target_file)) next
  
  coef_conf_file<-paste0('D:/work_data/raptor/unmarked_model_no_first_year_cov_1km_coefs_confs/',gsub('.RData','.csv',all_sp[s]))
  ...
  # pop<-npp<-shdi<-pasture<-LC_agri<-LC_sett<-0
  # 
  # env_cell_mean<-ddply(env,'cell',summarise,
  #                      pop=mean(pop),
  #                      npp=mean(npp),
  #                      shdi=mean(shdi),
  ...
  # Xs<-colnames(env_cell_mean)[-1]
  Xs<-c('pop','npp','shdi','LC_sett','LC_agri','pasture')
  
  list_dpsi.dXs<-list()
  for (v in 1:6) {
    # v=1
    # attach(env_cell_mean)
  ...
  # dpsi.dXs[[is.infinite(dpsi.dXs)]]<-NA
  ...
  # dpsi.dXs$cell<-env$cell
  detach()
  # sens_U<-func_sensitivity('U')
  # sens_L<-func_sensitivity('L')
  
  # colnames(sens_Est)<-paste0(colnames(sens_Est),'_Est')
  # colnames(sens_U)<-paste0(colnames(sens_Est),'_U')
  # colnames(sens_L)<-paste0(colnames(sens_Est),'_L')
  # sens_Est<-bind_cols(sens_Est,env_cell_mean)

  fwrite(dpsi.dXs,target_file)
